...
class DaemonConfig:
    ...
    auto_adjust_load: bool = True  # Adjust based on time of day
...
class DaemonRunner:
    ...
    def _get_load_profile(self) -> str:
        """Determine current load profile based on time."""
        now = datetime.now()
        hour = now.hour
        weekday = now.weekday()
    ...
    def _get_batch_size(self, config: DaemonConfig) -> int:
        ...
        profile = self._get_load_profile()
    ...
    def _run_batch(self, config: DaemonConfig, openai_client) -> None:
        """Run a single batch of simulation calls."""
        ...
        with self._metrics_lock:
            self._metrics.current_load_profile = self._get_load_profile()

        self._log(f"[BATCH] Starting batch: {operations_count} ops, {guardrails_count} guardrails ({self._metrics.current_load_profile} profile)")
        ...
        # Run guardrails
        for i in range(guardrails_count):
            if self._stop_requested:
                break
